...
html_content_1 = None
license_text = """
create_squad_tool - Version 10.0
...
# 创建Flask应用
app = Flask(__name__)  # 创建 Flask 应用实例
is_value_true = False  # 初始设为 False
...
def toggle_value():
    global is_value_true
    is_value_true = not is_value_true  # 切换值
    print(is_value_true)
    return jsonify({"success": True, "message": "停止建队！"})
...
def send_text():
    ...
    try:
        ...
        count = 0
        while True:
            if is_value_true == False:  # 暂停功能
                break
            pyautogui.press(first_key)
            pyautogui.hotkey("ctrl", "v")
            pyautogui.press("enter")
            count += 1
            #print(f"第 {count} 次建队操作成功！")

        play_sound(reverse=True)


        # 循环结束后返回最终成功的消息
        return jsonify({"success": True, "message": f"已建队: {text}，建队次数: {count}次。"})
...
def main():
    ...
    url_config_url = "https://gitee.com/petepetehiking/create_squad_tool/raw/master/config.txt"

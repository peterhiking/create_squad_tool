...
def send_text():
    global threshold, check_duration
    try:
        text = request.form[
            "text"
        ].strip()  # 获取表单中的文本数据，并去除两端的空白字符
        repeat_times = int(
            request.form["repeat_times"]
        )  # 获取选择的重复次数，转换为整数
        ...
        # 检测屏幕是否变黑
        if is_screen_black(threshold, check_duration):
            play_sound(reverse=True)
            # 输入有效时，进行原有的模拟键盘输入操作
        ...
        print(f"Error in Flask send_text route: {e}")
        return "Error occurred while processing the request."

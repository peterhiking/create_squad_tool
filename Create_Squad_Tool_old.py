...
def send_text():
    text = request.form["text"].strip()  # 获取表单中的文本数据，并去除两端的空白字符
    repeat_times = int(request.form["repeat_times"])  # 获取选择的重复次数，转换为整数
    key_press_order = request.form.get(
        "key_press_order", "home"
    )  # 获取用户选择的首个按键，默认为 'home'
    delay_time = float(request.form.get("delay_time", 0.1))

    # 定义检查输入有效性的条件
    def is_valid_input(input_text):
        # 检查文本是否为空或不满足特定条件（这里可以根据需要添加更多条件）
        # 例如：长度需大于3个字符，这个条件可以根据实际需求调整
        if len(input_text) < 3:
            return False
        # 如果有更多条件，可以在这里继续添加
        # 例如检查是否包含特定字符或是否符合格式等
        return True

    # 检查输入是否有效
    if not is_valid_input(text):
        # 如果输入无效，返回提示信息（这里简单返回到首页，实际使用中可以返回更友好的错误提示或者是保留用户输入让用户修改）
    ...
    for _ in range(repeat_times):  # 根据选择的重复次数执行操作
        pyautogui.press(first_key)  # 按下用户选择的第一个键
    ...
    return redirect(url_for("index"))  # 返回 HTML 内容
...
def copy_to_clipboard():
    content = "https://pan.baidu.com/s/1-rkawgOD0Ni0k2PzuRG0WA?pwd=f2v3"  # 替换成你要复制的具体内容
    clipboard.copy(content)
    messagebox.showinfo("成功", "百度网盘链接已复制到剪切板，请粘贴至浏览器地址栏进行下载。")
...
def main():
    ...
    url_config_url = "https://gitee.com/petepetehiking/create_squad_tool/raw/master/urls.txt"
    ...
    target_date = datetime(2024, 7, 14).date()
    ...
    def change_color():
        ...
        important_message_label.config(fg=current_color)
        color_index = (color_index + 1) % len(colors)  # 循环切换颜色
        root.after(3000, change_color)  # 1000毫秒后再次调用change_color
    change_color()

    # 添加按钮
    button = tk.Button(left_frame, text="复制最新版下载地址到剪切板", command=copy_to_clipboard)
    button.grid(row=5, column=0, columnspan=2, pady=(0, 0))
